...
Theme 将 Prompt 与输出样式分开。Prompt 使用浅蓝背景和蓝色边框，但不设置全局默认背景；状态栏也只使用蓝灰前景色，不使用分段背景色块，避免不同终端颜色能力下出现错位。scrollback 沿用终端背景，Agent 标题和工具使用蓝色，正文显式使用深黑色，成功使用绿色，错误使用红色。正文不依赖上一段 ANSI 状态，避免分批输出时意外继承标题颜色。样式键表达语义而不是具体控件，使未来更换终端库时仍能复用展示模型。
