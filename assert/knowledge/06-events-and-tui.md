...
Theme 将 Prompt 与输出样式分开。Prompt 使用浅蓝背景和蓝色边框，但不设置全局默认背景；状态栏也只使用蓝灰前景色，不使用分段背景色块，避免不同终端颜色能力下出现错位。scrollback 沿用终端背景，并以绿色表示成功、蓝色表示交互和工具、红色表示错误。样式键表达语义而不是具体控件，使未来更换终端库时仍能复用展示模型。
